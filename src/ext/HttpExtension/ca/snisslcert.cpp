...
)
{
    ...
    if (cCertificates)
    {
        ...
        {
            hr = WcaDoDeferredAction(PLATFORM_DECORATION(L"WixRollbackHttpSniSslCertsInstall"), sczRollbackCustomActionData, cCertificates * COST_HTTP_SNI_SSL);
            ExitOnFailure(hr, "Failed to schedule install SNI SSL certificate rollback");
            hr = WcaDoDeferredAction(PLATFORM_DECORATION(L"WixExecHttpSniSslCertsInstall"), sczCustomActionData, cCertificates * COST_HTTP_SNI_SSL);
            ExitOnFailure(hr, "Failed to schedule install SNI SSL certificate execution");
        }
        else
        {
            hr = WcaDoDeferredAction(PLATFORM_DECORATION(L"WixRollbackHttpSniSslCertsUninstall"), sczRollbackCustomActionData, cCertificates * COST_HTTP_SNI_SSL);
            ExitOnFailure(hr, "Failed to schedule uninstall SNI SSL certificate rollback");
            hr = WcaDoDeferredAction(PLATFORM_DECORATION(L"WixExecHttpSniSslCertsUninstall"), sczCustomActionData, cCertificates * COST_HTTP_SNI_SSL);
