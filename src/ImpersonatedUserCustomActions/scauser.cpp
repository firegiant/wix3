    ...
    )
{
    ...
    for (SCA_USER *psu = psuList; psu; psu = psu->psuNext)
    {
        ...
        else
        {
            ...
            WcaLog(LOGMSG_VERBOSE, "Failed to check existence of domain: %ls, user: %ls (error code 0x%x) - continuing", wzDomain, psu->wzName, HRESULT_FROM_WIN32(er));
        }
        ...
        if (WcaIsInstalling(psu->isInstalled, psu->isAction))
        {
            ...
            if ((USER_EXISTS_YES == ueUserExists) || (USER_EXISTS_INDETERMINATE == ueUserExists) || !(psu->iAttributes & SCAU_DONT_CREATE_USER))
            {
                ...
                hr = WcaDoDeferredAction(PLATFORM_DECORATION(L"FCreateUserRollback"), pwzRollbackData, COST_USER_DELETE);
            ...
            hr = WcaDoDeferredAction(PLATFORM_DECORATION(L"FCreateUser"), pwzActionData, COST_USER_ADD);
